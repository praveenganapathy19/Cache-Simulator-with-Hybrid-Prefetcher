...
//		printf("%u: %u\n",curr_cycle,cpu_status);
	...
	*/
	...
	fprintf(fp,"Total Run time is %u\n",curr_cycle);
	fprintf(fp,"D-cache total hit rate:%.4f\n",cpu.getHitRateL1());
	fprintf(fp,"L2 cache total hit rate:%.4f\n",cpu.getHitRateL2());
	fprintf(fp,"Avg. Mem Access Time:%.4f\n",cpu.getAMAT());
	fprintf(fp,"Average Memory Queue Size:%.4f\n",avgMemQ);
	fprintf(fp,"L2 BW Utilization:%.4f\n",L2BW);
	fprintf(fp,"Memory BW Utilization:%.4f\n",memBW);
